...
# Creating a Tiny Web Framework, __Due <strike>Wednesday, Oct 5th</strike> Thursday, Oct 6th, by 11PM__ 

(due date extended one day, coding starts at part 1)
...
## Skim Part 0, Start Coding with Part 1
...
The `Request` object simply represents an http request. It can take an http request as a string, parse out information about that request and expose that information as properties (such as `method`, `path`, and `headers`). It assumes HTTP/1.1
...
8. It should look something like <br> ![pic alt](../resources/img/hw03-02-evenWarmer-01-request.gif)
...
1. Copy everything from `evenWarmer.js` to `miniWeb.js`
